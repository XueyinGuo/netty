...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    public ChannelFuture bind(SocketAddress localAddress, ChannelPromise promise) {
        /*
        * TODO 这是把 pipeLine中所有的 handler 都要绑定端口吗？？？？
        *  */
        return pipeline.bind(localAddress, promise); /* 继续网队列中添加一步任务：把服务端注册的感兴趣事件 0 改为 16（ACCEPT） */
    }
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            /* 当前线程是否是 eventLoop 中的线程【eventLoop都有一个线程】 */
            if (eventLoop.inEventLoop()) {
                register0(promise);
            } else {
                try {
                    /*
                    * 把任务加到 eventLoop 的队列中，并且启动线程
                    * 【但此时线程还为 null， 之后调回到 ThreadExecutorMap 的匿名内部类中 】
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                doRegister();  /* 往 socketChannel注册感兴趣时间，暂时注册为 0，后边会有修改 */
                ...
                if (isActive()) { /* 查看当前的 ServerSocketChannel 是否已经绑定端口开始监听 和 是否处于开启状态 */
                    if (firstRegistration) {
                        pipeline.fireChannelActive(); /* ServerSocketChannel活跃状态则  触发通道ChannelActive事件 */
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (!wasActive && isActive()) {
                invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        pipeline.fireChannelActive();  /* 把服务端注册的感兴趣事件 0 改为 16（ACCEPT） */
                    }
                });
            }
        ...
        private void invokeLater(Runnable task) {
            try {
                ...
                eventLoop().execute(task); /* 往任务队列中继续添加异步任务，把服务端注册的感兴趣事件 0 改为 16（ACCEPT） */
