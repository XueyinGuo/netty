...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    protected SingleThreadEventExecutor(EventExecutorGroup parent, Executor executor,
        ...
        /*
        *
        * 把每个 EventLoop 的executor类型都设置为 ThreadPerTaskExecutor
        *
        * SingleThreadEventExecutor 的 executor 赋值为 ThreadExecutorMap 的匿名内部类对象
        * */
        this.executor = ThreadExecutorMap.apply(executor, this);
        /* 之后用来存放每个线程应该执行的任务，存放的都是 Runnable 对象 */
    ...
    * */
    ...
    public void execute(Runnable task) {
        ObjectUtil.checkNotNull(task, "task");
        /*
        * 把任务放进 EventLoop的任务队列，并为 此时还为空的线程赋值
        *
        * 1. register0任务 要先放进队列的，因为此时连线程还没有，没办法执行任务
        *       1.1 往 socketChannel注册感兴趣时间，暂时注册为 0，后边会有修改
        *
        *       1.2 【添加异步任务】给 EventLoop 中添加 ServerBootstrapAcceptor
        *       1.3【添加异步任务】 继续往任务队列中加入任务，这个任务是绑定端口
        * 4. 把服务端注册的感兴趣事件 0 改为 16（ACCEPT）
        * */
    ...
    private void execute(Runnable task, boolean immediate) {
        ...
        if (!inEventLoop) {
            /*
            * 启动线程，【此时线程为null】
            * 需要先去到 ThreadPerTaskExecutor 中进行线程创建
            * */
            startThread();
            if (isShutdown()) {
    ...
    private void startThread() {
        if (state == ST_NOT_STARTED) {
            /* CAS替换 NioEventLoop 的state值， 1 -> 2 */
    ...
    private void doStartThread() {
        assert thread == null;
        /*
         * executor 在一开始创建 EventLoopGroup 的时候 赋值为 ThreadExecutorMap 的匿名内部类对象
         * 所以此处跳入 ThreadExecutorMap 中
         * ThreadExecutorMap 中的  executor 为 ThreadPerTaskExecutor类型，
         *
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                thread = Thread.currentThread(); /* 为线程赋值为刚刚创建的线程 */
                if (interrupted) {
                    thread.interrupt();
                }
